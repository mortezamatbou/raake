    ...
    <div id="main" class="row">
        ...
        <div id="articles" class="col-lg-9 col-md-8 col-sm-8 col-xs-12">
            <h1 style="margin-top: 0px; padding: 5px 15px;border-radius: 2px; border: 1px solid rgba(0,0,0, .2); background-color: rgba(0,0,0, .02); font-size: 25px">
                نتایج جستجو برای دسته بندی <b><?= $name ?></b>
            </h1>
            ...
            <br>
                ...
                <?php
                $count = $article->get_category_page_count();
                if ($count != 0 && $count != 1) {
                    ?>
                        ...
                        <nav>
                            <ol class="pagination pagination-sm">
                                ...
                                <?php if ($article->pagination_current != 1) : ?>
                                    <li><a href="/category/<?= str_replace(' ', '-', $name)?>/<?= $article->pagination_current - 1 ?>" aria-label="Previous">&raquo;</a></li>
                                <?php endif; ?>
                                <?php
                                ...
                                for ($i; $i < $limit; $i = $i + 1) {
                                    if (($i + 1) == $article->pagination_current) {
                                        ...
                                        echo '<li><a href="/category/' . str_replace(' ', '-', $name) . '/' . ($i + 1) . '">' . ($i + 1) . '</a></li>';
                                    }
                                }
                                if ($article->pagination_current != $count) {
                                    echo '<li><a href="/category/' . str_replace(' ', '-', $name) . '/' . ($article->pagination_current + 1) . '" aria-label="Next">&laquo;</a></li>';
                                }
                                ?>
                            </ol>
                        </nav><br/>
